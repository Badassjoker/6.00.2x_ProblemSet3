...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        reprProb = self.maxBirthProb * (1 - popDensity)

        if random.random() < reprProb:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for a in range(1, 301):
    	...
    	averageRes.append(sum(reses) / float(numTrials))

    pylab.plot(range(300), averageRes, label = 'Average population line')
    pylab.title('Average populations, each step')
    pylab.xlabel('Step')
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if flag:
            ...
            if random.random() < reprProb:
            	...
            	for key, value in newResistances.items():
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for a in range(1, 301):
    	...
    	avgPops.append(sum(pops) / float(numTrials))
    	avgResistPops.append(sum(resistPops) / float(numTrials))

    pylab.plot(range(300), avgPops, label = 'Population line')
    pylab.plot(range(300), avgResistPops, label = 'Resistant population line')
    pylab.title('Virus population and resistant virus population')
    pylab.xlabel('Step')
    pylab.ylabel('Population')
    pylab.legend()
    pylab.show()
